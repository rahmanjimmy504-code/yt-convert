 ...
 *
 * Adapted from the YT Convert website (src/lib/faq-knowledge.ts),
 * dual-licensed by its copyright holder under the GNU General Public License
 * v3 or later for this repository.
 *
 * This is the assistant's entire brain in the app: answerLocally() is a pure
 * function with no network access, so the assistant works offline, needs no
 * API key, and sends your questions nowhere. An API key must never be shipped
 * inside an APK — the package is trivially unzipped and this source is public.
 *
...
export const KNOWLEDGE: KnowledgeEntry[] = [
  ...
  {
    id: "pwa-install",
    q: "How do I install and update the app?",
    a: "You are already using it. The app is distributed as an APK from the project's GitHub Releases page, and debug builds are attached to each GitHub Actions run. To update, download the newer APK and install it over the top — your history, favourites and preferences are kept. There is no Play Store listing, so nothing auto-updates; check Releases occasionally.",
    keywords: [
      "install",
      "pwa",
      "app",
  ...
  {
    id: "analytics",
    q: "What analytics does the app collect?",
    a: "None at all. The Android app has no analytics, no advertising SDK, no crash reporter and no cookie banner. Nothing about your usage is recorded or transmitted anywhere. The only network connections it makes are to the platform you pasted a link from, plus any converter site you deliberately open.",
    keywords: [
      "analytics",
      "privacy",
      "tracking",
  ...
  },
  {
    ...
    a: "No. Your recent lookups, favourite converter, format choice and dark-mode setting live in the app's local storage on this phone and never leave the device. Uninstalling the app deletes them. We operate no server for the app, so there is nothing for us to store even if we wanted to.",
    keywords: [
      "store links",
      "history",
      "localstorage",
  ...
  },
  {
    ...
    a: "1) Copy a link from YouTube, Spotify, TikTok, etc. 2) Paste it into YT Convert's box (or tap Paste from clipboard) and press Go. 3) Pick MP3 (audio) or MP4 (video). 4) If Download here is enabled, use it — the file is fetched straight to your phone and saved through MediaStore. Audio keeps its real container, usually M4A or WebM rather than MP3. 5) If first-party convert is unavailable or fails, click an AUTO-SEND converter or COPY NEEDED, then finish on that site.",
    keywords: [
      "how to use",
      "how does it work",
      "step by step",
  ...
  {
    id: "captcha",
    q: "Why is there no CAPTCHA in the app?",
    a: "There is no CAPTCHA because there is no shared server to protect. The website gates metadata lookups with Cloudflare Turnstile so bots cannot hammer its API on everyone else's behalf. In the app, every lookup and download runs on your own phone over your own connection, so there is no shared resource to abuse and nothing to verify. That also means no lookup limits and no rate limiting from us.",
    keywords: [
      "captcha",
      "turnstile",
      "verification",
  ...
  },
  {
    ...
    a: "Partly. The app itself, this assistant, the FAQ and all the legal screens work fully offline because they are bundled in the APK. Looking up a link and downloading media obviously still need an internet connection, since the file comes from the platform itself.",
    keywords: [
      "offline",
      "pwa offline",
      "service worker",
  ...
  },
  {
    ...
    a: "Not from us. The website rate-limits its API to keep a free server alive, but the app talks straight to the platform from your phone, so we impose no quotas. The platform itself can still throttle or block requests if you fetch a great many videos quickly — that is between your device and them.",
    keywords: [
      "limit",
      "rate limit",
      "too many requests",
  ...
  },
  {
    ...
    q: "Is this assistant a real AI? Does it use ChatGPT?",
    a: "It is a deterministic matcher, not a cloud language model. It scores your question against the app's built-in knowledge base and returns the best-matching documented answer, so it is instant, works with no signal, and cannot invent facts. Deliberately, it does NOT call ChatGPT, OpenAI or any other service: doing so would require shipping a secret API key inside the APK, where anyone could unzip it and spend the owner's money. The website version can optionally upgrade to a cloud model because its key stays on the server.",
    keywords: [
      "assistant",
      "ai",
      "chatgpt",
  ...
  },
  {
    ...
    a: "The website asks its server to look up metadata and proxy the file. The app does that work on your phone instead. Practical differences: no CAPTCHA, no rate limits, no analytics, downloads run over your own connection so they are not affected by the server being bot-blocked, progress appears as an Android notification you can cancel, files are saved through MediaStore, and audio is labelled honestly as M4A or WebM rather than MP3. The app also has no admin dashboard or status page, since there is no server to monitor.",
    keywords: [
      "difference",
      "app vs website",
      "vs site",
  ...
  {
    id: "open-source",
    q: "Is the app open source? What licence?",
    a: "Yes. The app is free software under the GNU General Public License v3 or later, and the complete source is published in the public yt-convert-android repository on GitHub. You may run, study, modify and redistribute it; if you distribute a modified version you must release your changes under the same licence and provide the source. The Licence screen in the app carries the full notices and the source link. Note the website itself stays under its own separate terms — only the app is GPL.",
    keywords: [
      "open source",
      "licence",
      "license",
  ...
  },
  {
    ...
    q: "Why is the Download here button greyed out?",
    a: "This build ships the interface and the native plugin skeleton only, so on-device extraction is not enabled yet and the button stays honestly disabled rather than failing when tapped. It lights up automatically in a later release once the NewPipeExtractor engine is added. Meanwhile you can use the Seal, YTDLnis or NewPipe handoff buttons for YouTube, or any converter card below.",
    keywords: [
      "download here",
      "greyed",
      "grayed",
  ...
  },
  {
    ...
    a: "Through Android MediaStore, which puts video in your Movies folder and audio in Music, so they appear automatically in your gallery or music player. Because MediaStore is used, the app does not need broad storage permission on modern Android. It does ask for notification permission so download progress can be shown and cancelled from the notification shade.",
    keywords: [
      "where saved",
      "where files",
      "download location",
...
const YT_KEYWORDS = [
  // App-specific vocabulary. Without these the off-topic guard swallows
  // perfectly on-topic questions such as "is it open source" or
  // "is this a real AI" before scoring ever runs.
  "assistant",
  "ai",
  "chatgpt",
  "openai",
...
export function answerLocally(questionRaw: string): AssistantResult {
  ...
  if (/(who are you|what are you|what is your name)/i.test(qLower)) {
    return {
      answer:
        "I'm the YT Convert assistant — a fully on-device helper that answers from the app's built-in documentation. I run entirely offline inside the app: no cloud AI, no API key, and your questions never leave your phone. I only answer about YT Convert: its features, supported platforms, how converters work, troubleshooting, privacy, legal notes, and installation. Ask me anything about the site and I'll cite the source FAQ.",
      sources: [],
      confidence: 0.9,
