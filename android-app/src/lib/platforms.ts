// Platform definitions ported from the website so the Android companion
// detects and labels the same links with the same rules.
...
/** Output format keys shared with the website. */
...
export function convertUnavailableReason(platform: PlatformKey): string {
  switch (platform) {
    ...
    case 'snapchat': return 'Snapchat does not expose a public media file we can pull on-device.';
