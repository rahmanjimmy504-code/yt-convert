...
// Platform definitions ported from the website (src/lib/platforms.ts) so the
// Android companion detects and labels the same links with the same rules.
// Kept pure (no DOM, no server) so it is unit-testable in the web bundle.
...
/**
 * Static first-party convert capability. True means the app will (once native
 * extraction lands) attempt to pull a public stream on-device. False is an
 * honest "not possible" (DRM catalog, or no public media URL).
 */
...
export function convertUnavailableReason(platform: PlatformKey): string {
  switch (platform) {
    ...
    case 'snapchat':
      return 'Snapchat does not expose a public media file we can pull on-device.';
    case 'br':
