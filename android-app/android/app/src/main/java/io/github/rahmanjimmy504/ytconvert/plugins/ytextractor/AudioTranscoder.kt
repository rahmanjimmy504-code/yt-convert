...
object AudioTranscoder {
    ...
    fun transcode(
        ...
        try {
            ...
            val durationUs = if (source.format.containsKey(MediaFormat.KEY_DURATION)) {
                source.format.getLong(MediaFormat.KEY_DURATION)
            } else {
                -1L
            }
    ...
    private class FlacSink(private val handle: MediaStoreSaver.RawHandle, private val sampleRate: Int, private val channels: Int) : PcmSink {
        ...
        init {
            handle.write(AudioTranscoder.FLAC_MAGIC_BYTES, 0, 4)
            ...
            val format = MediaFormat.createAudioFormat(AudioTranscoder.MIME_FLAC, sampleRate, channels)
            format.setInteger(AudioTranscoder.KEY_FLAC_COMPRESSION_LEVEL, 5)
            encoder = MediaCodec.createEncoderByType(AudioTranscoder.MIME_FLAC)
        ...
        private fun feedEncoder(buffer: ByteBuffer, frames: Int) {
            // Read the block into a ShortArray: MediaCodec input buffers are
            // direct ByteBuffers whose asShortBuffer() view is not
            // array-backed, and ShortBuffer has no offset/length put overload.
    ...
    private class OpusSink(
        ...
        init {
            val format = MediaFormat.createAudioFormat(AudioTranscoder.MIME_OPUS, encoderRate, channels)
            format.setInteger(MediaFormat.KEY_BIT_RATE, bitrateBps)
            encoder = MediaCodec.createEncoderByType(AudioTranscoder.MIME_OPUS)
            encoder.configure(format, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE)
            encoder.start()
        ...
        private fun maybeStartMuxer() {
            ...
            val trackFormat = MediaFormat.createAudioFormat(AudioTranscoder.MIME_OPUS, AudioFormats.OPUS_SAMPLE_RATE, channels)
    ...
    // Direct members of the AudioTranscoder object: AudioTranscoder is a
    // singleton (object), so these live at object scope and the nested sink
    // classes reference them as AudioTranscoder.*.
