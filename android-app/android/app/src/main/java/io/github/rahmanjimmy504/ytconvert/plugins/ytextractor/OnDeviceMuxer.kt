...
/**
 * Stream-copy remux for YouTube's adaptive H.264 MP4 + AAC tracks.
 *
 * MediaExtractor reads both allowlist-checked CDN URLs directly and
 * MediaMuxer writes directly to the pending MediaStore/file target. There is
 * no decoded frame, re-encode, or intermediate file: only compressed samples
 * move from the two source containers into one MP4 container.
 */
object OnDeviceMuxer {
    ...
    private fun openTrack(url: String, mimePrefix: String): SourceTrack {
        ...
        try {
            ...
            throw IllegalStateException("The adaptive source has no ${mimePrefix.removeSuffix("/")} track.")
        } catch (e: Exception) {
            extractor.release()
            throw e
        }
