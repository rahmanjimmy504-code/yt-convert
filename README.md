...
| `APIFY_RUN_TIMEOUT_S` | `300` | Per-run timeout in seconds (clamped to 30–300), sized to the Actor's ~170-second retry ladder. Bounds run DURATION; one request performs exactly one Actor run — no Actor retries or fan-out |
| `APIFY_MAX_TOTAL_CHARGE_USD` | `0.50` | Per-run charge ceiling in USD, sent to Apify as the sync run's `maxTotalChargeUsd` URL query parameter (never inside the Actor JSON input). Apify aborts a run whose charge would exceed this. `0` omits the parameter (no per-run ceiling; the monthly cap still applies) |
| `APIFY_ACTOR_BUILD` | *(empty)* | Optional Actor build tag/id to pin runs to. Unset = no `build` parameter is sent and runs follow the Actor's default build. Note: the verified working build is tag `0.0.64` (Apify build 64, three-part semver — not `0.064`); neither is default-pinned. Pin only after re-verifying a build |
...
| `APIFY_RESIDENTIAL_PROXY_MODE` | *(empty)* | Optional paid Apify proxy opt-in. Only `fallback` is accepted and sent as Actor input; unset or anything else omits `residentialProxyMode`. Residential traffic is **$0.05/MB when used** — one ~80 MB 720p file can cost about **$4**, nearly the whole $5 free credit |
...
4. **AHM7xMakki AllDL** — one key-less `GET ahm7xmakki.com/api/alldl?url=…` call that converts on the operator's own egress and hands back a finished file on its CDN host `c.ymcdn.org` (downloads redirect within the operator's own `ymcdn.org` domain, which is allowlisted as a suffix for that reason). Exactly one timeout-bounded attempt per request; the candidate's first bytes are sniffed so a mislabelled or HTML body is discarded, and any failure falls through non-fatally.
5. **Cobalt** (last free resort) — the operator's own instance (`COBALT_API_URL`) first, then up to three **reviewed** public instances that `cobalt.directory` currently reports as passing its YouTube test. The resulting muxed mp4 / mp3 is used. See the caveat below.
6. **Apify Actor** (paid, opt-in) — one bounded `run-sync-get-dataset-items` run of `APIFY_ACTOR_ID` (yt-dlp on Apify's own egress, which is why it can succeed when this server's IP is bot-blocked). On normal requests it remains the absolute last resort after everything above — including cobalt — returned nothing. On a confirmed BotGuard IP wall, it is tried immediately after Innertube (mirrors are skipped as before) so Cloudflare's request budget reaches Apify before the slow free farms; if it fails, the request then continues to 9Convert, AllDL, and cobalt. It runs only when `APIFY_TOKEN` is set and only while the account's monthly usage is under `APIFY_MONTHLY_CAP_USD`. See the caveat below and [docs/apify-provider.md](docs/apify-provider.md).
...
**Honest limitation:** re-verified 2026-09-01 — every public instance passing the directory's YouTube test that day (`api.cobalt.rpkiinval.id`, `cobaltapi.squair.xyz`, `cobalt-omega.wolfy.love`, plus the seven previously reviewed hosts) still advertises a `turnstileSitekey`, meaning it only issues Bearer tokens to clients that solved a Cloudflare Turnstile challenge in a browser. The one no-Turnstile instance (`cobaltapi.cjs.nz`) fails the directory's own YouTube check (`error.api.youtube.login`), so it cannot be reviewed in. Server-to-server calls therefore usually return `error.api.auth.turnstile.missing`. The candidates are still tried — the check is one bounded request and instance policies change often — but this app does **not** solve challenges. The official `api.cobalt.tools` and `*.imput.net` instances are deliberately excluded, because the cobalt docs state hosted instances are not intended for use by other projects without permission. A reliably working fallback still means pointing `COBALT_API_URL` at an instance you run.
...
The Apify step is **opt-in and costs real credit**, so it is fenced in on every axis: it runs only when `APIFY_TOKEN` is set, normally only after the entire free chain (including cobalt) produced nothing, and earlier only for the confirmed bot-wall fast path described above. It still runs exactly once per request, only while the account's monthly usage — read live from `GET /v2/users/me/limits` before every run — is under `APIFY_MONTHLY_CAP_USD` (the check fails closed: no answer means no run), with a run timeout (`APIFY_RUN_TIMEOUT_S`, default 300 seconds, sized to the Actor's ~170-second retry ladder) bounding run DURATION and a per-run charge ceiling (`APIFY_MAX_TOTAL_CHARGE_USD`, default `0.50`, sent as the run URL's `maxTotalChargeUsd` query parameter) bounding the single run's total charge in dollars. At or over the cap the request simply falls through to the normal "try a converter" message. Every skip, start, success, and failure logs a `[apify]` line for Cloudflare diagnostics.

The Actor stores the finished file in the run's key-value store and returns a `downloadUrl` on `api.apify.com`. That one host — exact match, never a suffix rule — is added to the media allowlist only while `APIFY_TOKEN` is set, and `extract.ts` re-checks every URL against the allowlist before the convert proxy can fetch it. The operator's token is attached to the `api.apify.com` download URL server-side so a non-public store can still be read; it never reaches the browser, because `/api/convert` streams the bytes through this app. If an Actor ever serves files from a different host, the operator must name it explicitly in `APIFY_PROXY_HOSTS` (exact hosts only). `residentialProxyMode` is omitted by default; the only opt-in is `APIFY_RESIDENTIAL_PROXY_MODE=fallback`, which can add $0.05/MB when Apify needs residential traffic (about $4 for one ~80 MB 720p video). An HTTP 403 from the Apify API is an account/token or Actor-permission problem, not a YouTube proxy problem; residential mode cannot fix it. If the Actor itself reports a residential-proxy 403, turn the setting off unless the account is entitled to that paid proxy product. See [docs/apify-provider.md](docs/apify-provider.md) for the operator guide.

**Optional signed-in runs (`APIFY_YOUTUBE_COOKIES`).** The Actor accepts a Netscape-format `cookies.txt` (`youtubeCookies` input) and uses it for every download attempt. Export one from a **throwaway** YouTube account (a "Get cookies.txt" browser extension) and set it as a Secret next to `APIFY_TOKEN`; the app validates the shape (a pasted `NAME=value; …` header is rejected — yt-dlp would silently ignore it and run anonymously anyway), caps it at 64 KiB, and bridges it verbatim into the run input. It travels only to `api.apify.com` over HTTPS with the operator's Bearer token, is never logged or cached, and never reaches the browser. An unusable value never disables the fallback — it just runs without cookies (with one `[apify]` warning in the logs).
...
- **YouTube / YT Music:** runs two requests in parallel — the YouTube oEmbed endpoint and a fallback chain over public Invidious instances (`invidious.tiekoetter.com`, `invidious.f5.si`, `yt.chocolatemoo53.com`, `invidious.nerdvpn.de`), tried in order until one responds with `200`. oEmbed supplies `title`, `author_name`, `thumbnail_url`; Invidious supplies `lengthSeconds`, `viewCount`, `published`, and backs up `title`/`author` if oEmbed came back empty.
