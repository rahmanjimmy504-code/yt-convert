...
The Apify step is **opt-in and costs real credit**, so it is fenced in on every axis: it runs only when `APIFY_TOKEN` is set, normally only after the entire free chain (including cobalt) produced nothing, and earlier only for the confirmed bot-wall fast path described above. It still runs exactly once per request, only while the account's monthly usage — read live from `GET /v2/users/me/limits` before every run — is under `APIFY_MONTHLY_CAP_USD` (the check fails closed: no answer means no run), and with a run timeout (`APIFY_RUN_TIMEOUT_S`) bounding the per-minute bill. At or over the cap the request simply falls through to the normal "try a converter" message. Every skip, start, success, and failure logs a `[apify]` line for Cloudflare diagnostics.
