...
4. **AHM7xMakki AllDL** — one key-less `GET ahm7xmakki.com/api/alldl?url=…` call that converts on the operator's own egress and hands back a finished file on its CDN host `c.ymcdn.org`. Exactly one timeout-bounded attempt per request; the candidate's first bytes are sniffed so a mislabelled or HTML body is discarded, and any failure falls through non-fatally.
