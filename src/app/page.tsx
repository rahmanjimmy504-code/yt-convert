...
function getPlatform(u) {
  ...
  if (/spotify\.com|open\.spotify/i.test(u)) return 'spotify';
  return null;
}
...
function sGet(k, f) { if (typeof window === 'undefined') return f; try { return localStorage.getItem(k) || f; } catch { return f; } }
function sSet(k, v) { if (typeof window === 'undefined') return; try { localStorage.setItem(k, v); } catch {} }
function sGetJ(k, f) { if (typeof window === 'undefined') return f; try { return JSON.parse(localStorage.getItem(k) || 'null') || f; } catch { return f; } }
function sSetJ(k, v) { if (typeof window === 'undefined') return; try { localStorage.setItem(k, JSON.stringify(v)); } catch {} }
...
export default function Home() {
  ...
  useEffect(() => {
    setMounted(true);
    const s = sGet('yt-convert-dark', null);
  ...
  const toggleDark = () => setDark(d => { const n = !d; document.documentElement.classList.toggle('dark', n); sSet('yt-convert-dark', n ? '1' : '0'); return n; });
  useEffect(() => { if (!mounted) return; const i = setInterval(() => setTipIdx(t => (t + 1) % tips.length), 5000); return () => clearInterval(i); }, [mounted]);
  ...
  const all = [
    ...
    { name: 'SpotifyDown', url: 'https://spotify-downloader.com/', desc: 'Spotify tracks to MP3.', color: 'from-green-500 to-green-600', platform: ['spotify'], recommended: true },
  ];
  ...
  const clearHist = () => { setHistory([]); sSetJ('yt-convert-history', []); };
  ...
  if (!mounted) return <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-950 dark:to-gray-900"><div className="w-8 h-8 border-[3px] border-red-500 border-t-transparent rounded-full animate-spin"/></div>;
            ...
            <div className="grid grid-cols-5 gap-3">
              ...
              <div className="flex flex-col items-center gap-1.5 group">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                  <svg width="22" height="22" viewBox="0 0 24 24"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z" fill="white"/></svg>
                </div>
