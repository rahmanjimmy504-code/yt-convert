...
- YT Convert converts where it legally and technically can: Download here extracts a public stream (YouTube/YT Music via Music-first Innertube, raced Piped/Invidious/embed mirrors, then the public 9Convert/dlsrv farm; SoundCloud progressive; public X/TikTok/Instagram/Facebook URLs) and proxies it. Files are never stored. A YouTube “Sign in to confirm you’re not a bot” error is a BotGuard challenge on the website server’s IP; if automatic fallbacks fail, tell the visitor to use the 9Convert card below, never to run a PO-token server or Termux. DRM catalogs (Spotify, Deezer, Apple Music, Amazon Music) are not ripped — preview only / use a licensed downloader. Private, members-only, deleted, and region-blocked videos are not unlocked, and not every YouTube video works. Snapchat and BeReal have no public media URL. Third-party converter cards are the fallback when extraction fails. Do not claim every converter receives the URL automatically. AUTO-SEND = verified deep link/form. COPY NEEDED = honest copy step.
...
1) Copy link 2) Paste in box (auto-fetch) or Go 3) Choose MP3/MP4 4) Use Download here when shown 5) Otherwise click AUTO-SEND or COPY NEEDED. If COPY NEEDED, paste after the copy step.
